document.addEventListener('DOMContentLoaded', () => {
  ...
  async function cargarTorneos() {
    try {
      const res1 = await fetch('../torneo/process-torneo.php?id=');             // torneos propios
      torneosPropiosData = await res1.json();
      console.log(torneosPropiosData);
      const res2 = await fetch('../torneo/process-torneos-otros.php');      // torneos no inscripto
...
function crearCardTorneo(t) {
  ...
  if (t.inscripta > 0) {
    ...
    btnSumarme.addEventListener('click', async () => {
      try {
        console.log("aca");
        const res = await fetch('../torneo/procesar-solicitud.php', {
          method: 'POST',
  ...
  async function verJugadores(idTorneo) {
    try {
      ...
      const res = await fetch('../torneo/procesar-ver-jugadores.php', {
        method: 'POST',
        body: formData
      });
 ...
 closeDenuncia.addEventListener('click', () => {
  ...
  btnConfirmar.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('../torneo/procesar-denuncia.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
